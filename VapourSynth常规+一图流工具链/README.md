...
| AAC_AUDIO_PATH            | 如果下载下来的视频 / 音频文件已经是 AAC 编码，则可以直接将视频或音频文件的路径填到这里。（ **例：** 从 YouTube 下载时格式选择了 AAC；某网站上下载的广播原本就是 AAC 编码） |
...
| BASE_VIDEO_GOP_LENGTH     | 一图流视频的 GoP（Group of Pictures）长度。该值越大，则压出的视频大小就越接近音频大小（即视频部分几乎不占空间），但是 Aegisub 等软件打开时就越卡。 |
